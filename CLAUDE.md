...
- Backend é **Supabase** (Postgres + RLS + RPCs). O app é SPA static export e fala **direto** com o Supabase via `@supabase/ssr` (`getSupabaseBrowserClient`, `src/shared/lib/supabase/`). **Não há MSW** em produção — todos os dados (turmas, alunos, chamadas, presenças) vêm do banco real; MSW só mocka nos testes.
