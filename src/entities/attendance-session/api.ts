...
export async function fetchAttendanceSessionsByGroup(groupId: string): Promise<AttendanceSession[]> {
 ...
 */
export async function createAttendanceSession(input: NewAttendanceSession): Promise<AttendanceSession> {
  ...
  await mutateCollection<AttendanceSession>("attendanceSessions", (rows) => {
    const exists = rows.some(
      (row) => row.groupId === session.groupId && row.date === session.date,
    );
