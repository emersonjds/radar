...
import { attendanceRate } from "@/features/analytics/model";
import { formatDateLong, formatPercent } from "@/shared/lib/format";
...
const ACESSO_RAPIDO = [
  { label: "Alunos", href: "/students", icon: "user" as const },
  { label: "Minhas chamadas", href: "/attendance", icon: "session" as const },
];

const RELATORIOS_RECENTES = [
  { name: "Resultado_prova_final.pdf", meta: "Gerado há 2h • 4.2 MB" },
  { name: "Frequência_semanal.xlsx", meta: "Gerado há 5h • 120 KB" },
];
...
export function TeacherDashboard() {
  ...
  const hoje = formatDateLong(new Date().toISOString().slice(0, 10));
          ...
          <Card>
            ...
            <ul className={styles.relatoriosLista}>
              {RELATORIOS_RECENTES.map((relatorio) => (
                <li key={relatorio.name} className={styles.relatorioItem}>
                  <Icon name="relatorios" />
                  <div className={styles.relatorioInfo}>
                    <span className={styles.relatorioNome}>{relatorio.name}</span>
                    <span className={styles.relatorioMeta}>{relatorio.meta}</span>
                  </div>
                  <IconButton label={`Baixar ${relatorio.name}`}>
